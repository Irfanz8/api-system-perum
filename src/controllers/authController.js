...
export const handleOAuthCallback = async (req, res) => {
  try {
    ...
    try {
      ...
      userRole = user.user_metadata?.role || ROLES.USER;
    }
...
export const signUp = async (req, res) => {
  try {
    ...
    if (data.user) {
      try {
        const roleResult = await db`SELECT role FROM users WHERE id = ${data.user.id}`;
        if (roleResult.length > 0 && roleResult[0].role) {
          userRole = roleResult[0].role;
        }
      } catch (dbError) {
        console.error('Error fetching role from database:', dbError);
        userRole = data.user.user_metadata?.role || ROLES.USER;
      }
    }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    // Get actual role from database
    let updatedUserRole = ROLES.USER;
    try {
      const roleResult = await db`SELECT role FROM users WHERE id = ${userId}`;
      if (roleResult.length > 0 && roleResult[0].role) {
        updatedUserRole = roleResult[0].role;
      }
    } catch (dbError) {
      console.error('Error fetching role from database:', dbError);
      updatedUserRole = data.user.user_metadata?.role || ROLES.USER;
    }
